    ...
    if (typeof showLectureResults === 'function') {
        ...
        setTimeout(() => {
            ...
            if (sourceContainer && targetContainer) {
                ...
                setTimeout(() => {
                    if (typeof initLectureResultAudioListeners === 'function') {
                        initLectureResultAudioListeners();
                    }
                    const highlightedWords = document.querySelectorAll('.lecture-keyword-highlight');
                    highlightedWords.forEach(word => {
                        word.addEventListener('mouseenter', showLectureTooltip);
                        word.addEventListener('mouseleave', hideLectureTooltip);
                    });
                }, 100);
        ...
        // showLectureResults 함수가 없으면 임시 메시지
        const targetContainer = document.getElementById('finalDetailContent');
        if (targetContainer) {
            targetContainer.innerHTML = '<p style="text-align: center; color: #666; padding: 40px;">Lecture 상세 해설은 준비 중입니다.</p>';
        }
    }
}
