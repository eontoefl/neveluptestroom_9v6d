...
function renderLectureSetResult(setResult, setIdx) {
    ...
    // 오디오 재생 (첫 번째 answer의 audioUrl 사용)
    if (setResult.answers && setResult.answers[0] && setResult.answers[0].audioUrl) {
        const audioUrl = convertGoogleDriveUrl(setResult.answers[0].audioUrl);
        html += `
    ...
    // 스크립트 렌더링 (첫 번째 answer의 스크립트 사용)
    if (setResult.answers && setResult.answers[0]) {
        const firstAnswer = setResult.answers[0];
        html += renderLectureScript(
            firstAnswer.script,
            firstAnswer.scriptTrans,
            firstAnswer.scriptHighlights || []
        );
    }
...
function renderLectureAnswer(answer, qIdx, setIdx) {
    ...
    answer.options.forEach((option, optIdx) => {
        ...
        const isUserAnswer = answer.userAnswer === optionNum;
        ...
        // 선택지 번역
        if (answer.optionTranslations && answer.optionTranslations[optIdx]) {
            html += `
            <div class="option-translation">
                ${escapeHtml(answer.optionTranslations[optIdx])}
            </div>
            `;
        }
        
        // 선택지 설명
        if (answer.optionExplanations && answer.optionExplanations[optIdx]) {
            html += `
            <div class="option-explanation">
                <strong>설명:</strong> ${escapeHtml(answer.optionExplanations[optIdx])}
            </div>
            `;
        }
    });
