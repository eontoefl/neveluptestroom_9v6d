...
class AnnouncementComponent {
    ...
    parseCSV(csvText) {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const setData = {
                ...
                scriptHighlight: columns[5].trim(),
                questions: [
                    {
    ...
    /**
     * 오디오 시퀀스 재생
     * 2초 대기 → 나레이션 → 2초 대기 → 공지사항 오디오
     */
    playAudioSequence() {
        console.log('[AnnouncementComponent] 오디오 시퀀스 시작');
    ...
    playMainAudio(onEnded) {
        ...
        if (!audioUrl) {
            console.warn('[AnnouncementComponent] 공지사항 오디오 URL 없음, 스킵');
            if (onEnded) onEnded();
            return;
        }
    ...
    submit() {
        ...
        const resultData = {
            ...
            scriptHighlight: this.currentSetData.scriptHighlight,
            ...
            sets: [
                {
                    ...
                    scriptHighlight: 'demo,announcement',
                    questions: [
                        {
